...
def main():
    df = pd.read_csv("BTC.csv", delimiter=',', encoding="utf-8-sig", index_col=False, nrows=254)  # nrows kaç tane veriyi incelediğini gösteriyor onu kullanıcıya seçtirebiliriz mi?
    df = df.iloc[::-1]
    ...
    # Chart updates
    fig.update_layout(title=str(df['symbol'][0] + ' Daily Chart'), hovermode='x', dragmode="zoom", width=1800,
                      height=1350,
